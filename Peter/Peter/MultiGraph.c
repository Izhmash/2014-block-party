...
task main()
{
  ...
  while(true)
  {
   ...
   // else
    ...
    if(!(nNxtButtonPressed == kButtonLeft) && LeftPressed == true)
    {
      LeftPressed = false;
      mode = IR;
    }

    if(nNxtButtonPressed == kButtonRight)
    {
      RightPressed = true;
    }
    if(!(nNxtButtonPressed == kButtonRight) && RightPressed == true)
    {
      RightPressed = false;
      mode = LITE;
    }
    ...
    switch(mode)
  {
    case LITE: y = 64.00000*(LightValue/100.00);
    case IR: y = 64.00000*(IRValue/255.00);
  }
    //nxtSetPixel(n,y);
    if(time100[T1]==1)
    {
