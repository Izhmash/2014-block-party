...
#pragma config(Motor,  motorB,          motorR,        tmotorNormal, PIDControl, encoder)
  ...
  // Read the current non modulated signal direction
    ...
    if (!HTIRS2readAllACStrength(IR, acS1, acS2, acS3, acS4, acS5 ))
     ...
     // {
      ...
      while( Sensor < 5 && Sensor > 0)
      {
        if(dc[Sensor]>7)
        {
        motor[motorR] = 100-(dc[Sensor]/255 * 100);
        }
        else
        {
          motor[motorR] = 0;
      ...
      while( Sensor > 5)
      {
        if(dc[Sensor]>7)
        {
        motor[motorL] = 100-(dc[Sensor]/255 * 100);
        }
        else
        {
          motor[motorL] = 0;
